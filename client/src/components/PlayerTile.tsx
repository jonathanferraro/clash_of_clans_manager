...
export function PlayerTile({ data }: { data: PlayerData }) {
    ...
    return (
        <div className="card mb-4 shadow border-light" style={{ backgroundColor: 'var(--bs-card-bg)', transition: 'box-shadow 0.3s ease-in-out' }}>
            {/* Card Header*/}
            <div className="card-header bg-body-tertiary d-flex flex-column flex-sm-row justify-content-sm-between align-items-start align-items-sm-center py-3 px-sm-4 px-3 border-bottom-0">
                <div className="d-flex flex-column flex-md-row align-items-md-center gap-md-3 mb-2 mb-sm-0 me-sm-3"> 
                    <h5 className="mb-1 mb-md-0 text-primary fw-bold text-break">{data.name}</h5>
                    <span className="badge text-bg-secondary text-uppercase px-2 py-1 align-self-start align-self-md-center flex-shrink-0"> 
                        {data.role === 'admin' ? 'elder' : data.role}
                    </span>
                </div>
                <div className="d-flex flex-wrap align-items-center gap-x-3 gap-y-1 text-muted small"> 
                    <span className="text-nowrap"><i className="bi bi-star-fill me-1"></i>Level {data.level}</span>
                    <span className="text-nowrap mx-2"><i className="bi bi-house-fill me-1"></i>TH{data.town_hall}</span>
                    <span className="text-nowrap mx-2"><i className="bi bi-trophy-fill me-1 text-warning"></i>{data.trophies}</span>
                </div>
            </div>
            ...
            <div className="card-body p-sm-4 p-3"> 
                <div className="row gy-3">
                    ...
                    <div className="col-md-6">
                        <ul className="list-unstyled mb-0">
                            <li className="mb-2 d-flex flex-wrap justify-content-between align-items-center gap-x-2 gap-y-1">
                            ...
                            <li className="d-flex flex-wrap justify-content-between align-items-center gap-x-2 gap-y-1">
                ...
                {/* Recent War Stats */}
                 <div className="mb-3 d-flex flex-wrap justify-content-between align-items-center gap-x-2 gap-y-1">
                        ...
                        {playerLast5Wars && playerLast5Wars.length > 0 && !isLoading && !error && (
                            <div className="list-group list-group-flush">
                                {playerLast5Wars.map((war, index) => (
                                    <div key={war.war_id || index} className="list-group-item bg-light px-sm-3 px-2 py-2 mb-2 rounded border"> {/* Adjusted padding */}
                                        {war.attacks && war.attacks.length > 0 ? (
                                            <>
                                                <div className="d-flex flex-wrap justify-content-between align-items-center mb-1 gap-x-2 gap-y-1">
                                                    <span className="small fw-medium">Attack 1:</span>
                                                    <div className="d-flex align-items-center flex-wrap gap-1">
                                                        <span className="me-2 text-nowrap">{renderStars(war.attacks[0].stars)}</span>
                                                        <span className="badge bg-secondary rounded-pill">{war.attacks[0].destruction_percentage}%</span>
                                                    </div>
                                                </div>
                                                {war.attacks.length > 1 && (
                                                     <div className="d-flex flex-wrap justify-content-between align-items-center gap-x-2 gap-y-1">
                                                        <span className="small fw-medium">Attack 2:</span>
                                                        <div className="d-flex align-items-center flex-wrap gap-1">
                                                            <span className="me-2 text-nowrap">{renderStars(war.attacks[1].stars)}</span>
